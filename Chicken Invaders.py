...
class Game:
    def __init__ (self):
        ...
        self.VPlanes = 5  # Tốc độ Planes
        self.VEnemy = 1  # Tốc độ Enemy
        self.scores = 0  # Điểm số
        self.high_scores=0  #Điểm cao
        self.numberEnemy = 1  # Số lượng enemy trong một screen
        ...
        self.screen = pygame.display.set_mode((self.xScreen, self.yScreen))  # Khởi tao kích thước màn hình
    ...
    def show_highscore(self, x, y, high_scores, size):  # Hiển thị điểm cao 
        font = pygame.font.SysFont("comicsansms", size)
        high_score = font.render(str(high_scores), True, (255, 255, 255))
        self.screen.blit(high_score, (x, y))
    ...
    def bullet(self):
        for count, i in enumerate(self.listBullet):
            ...
            self.image_draw('image/bullet.png', xBullet,
    ...
    def run(self):
        while self.gamerunning:
            ...
            if self.YGameOver > self.yScreen-50:  # Nếu Enemy về gần đích
                ...
                while(True):
                    ...
                    if(newGame == True):  # Thoát vòng while để vào game mới
                        break
                    if self.scores > self.high_scores: # Update điểm cao
                        self.high_scores = self.scores
                    self.show_score(100, 100, "Scores:{}".format(self.scores), 40)  # In điểm
                    self.show_highscore(600,100, "High Scores:{}".format(self.high_scores),40) #In điểm cao 
            ...
            self.image_draw("image/ptit.jpg",self.xScreen-180, 10, 150, 60)  # Logo code Learn
            self.enemy()
            self.bullet()
